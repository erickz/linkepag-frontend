...
import { maskPriceInput, parsePrice, formatUrl } from '@/lib/masks';
          ...
          {activeLinks.length === 0 ? (
            ...
            activeLinks.map(link => (
              <div key={link.id} className={`h-8 rounded-lg flex items-center px-3 text-xs ${link.isPaid ? `bg-gradient-to-r from-slate-800 to-slate-900 text-white ${accent.borderClass}` : 'bg-white text-slate-700 border border-slate-200'}`}>
                ...
                {link.isPaid && <span className={`font-bold ${accent.textClass}`}>R$ {link.price?.toFixed(2)}</span>}
              </div>
            ))
          )}
                ...
                return (
                  <div key={link.id} className={`group flex items-center gap-2 p-3 rounded-xl border transition-all ${link.isPaid ? 'border-amber-200 bg-amber-50/30' : 'border-slate-200 hover:border-indigo-300'} ${!link.isActive ? 'opacity-50' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs">{link.isPaid ? <span className="font-bold text-amber-600">R$ {link.price?.toFixed(2)}</span> : <span className="text-slate-400">Gratuito</span>}<span className="text-slate-300">•</span><span className="text-slate-400 truncate">{link.url.replace(/^https?:\/\//, '').replace(/^www\./, '')}</span></div>
                    </div>
